...
def get_stock_data_alpha_vantage(ticker):
    ...
    Includes historical data for comprehensive AI analysis
    """
    if not ALPHA_VANTAGE_API_KEY:
...
def ask_llm_about_sector(sector_data, user_question):
    ...
    if client is None:
        try:
            ...
            return f"⚠️ OpenAI API key not configured. Please set OPENAI_API_KEY in your .env file or HF Spaces secrets."
...
def main():
    ...
    try:
        # Check API keys but don't fail if missing (will be set in deployment secrets)
        openai_key = os.getenv("OPENAI_API_KEY")
        if not openai_key:
            print("⚠️  OPENAI_API_KEY not found in environment")
            print("   For local use: set it in .env file")
            print("   For deployment: set it in Gradio/HF Spaces secrets")
